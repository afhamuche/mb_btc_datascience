...
if args>1:
    ...
    filename = input("Enter historical filename .csv: ")

histogram = 'histogram_price_' + filename
...
with open(histogram, 'r') as ifile:
    ...
    for i in range(len(buy_x_train)):
        buy_x_train[i] = float(buy_x_train[i])
        buy_y_train[i] = int(buy_y_train[i])
        sell_x_train[i] = float(sell_x_train[i])
    ...
    for i in range(3):
        ...
        print(f"(w,b) found by gradient descent: ({w_final:.5f},{b_final:.5f})")
